...
stopCluster(cluster)
print(RF_AL_mod)
RF_AL_pred <- predict(RF_AL_mod,testing)
RF_AL_testing <- testing
RF_AL_testing$predRight <- predict(RF_AL_mod,testing, "raw")
RFcm <- confusionMatrix(RF_AL_testing$predRight, RF_AL_testing$classe)
print(RFcm)
...
####3. Expected out of sample error
 ...
 And For the Random Forest Model I found the calculated Accuracy to be: `r RFaccuracy`, a bit below .99, but the model did predict the train data correctly.
 
## Results
The Model, using data directly related to the sensors and using Random Forest with CV set at 5 work well.  With parellel processing and cross validation in trainControl set to "CV" with a value of 5, the computing time was respectable on my system at an elapsed time of 211. 
...
system.time(LDA_AL_pred <- predict(LDA_AL_mod,testing))

LDA_AL_testing <- testing
LDA_AL_testing$predRight <- predict(LDA_AL_mod,testing, "raw")
confusionMatrix(LDA_AL_testing$predRight, LDA_AL_testing$classe)  
```
####Boosting  
```{r eval = FALSE}
...
stopCluster(cluster)
print(RF_AL_mod)
system.time(RF_AL_pred <- predict(RF_AL_mod,testing))
